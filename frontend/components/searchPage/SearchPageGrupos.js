...
export default function SearchPageGrupos() {
  const router = useRouter();
  const [searchParams, setSearchParams] = useState({});
  const [currentPage, setCurrentPage] = useState(1);
  
  const { data: results, totalPages, loading, error } = useGrupos({
    page: currentPage,
    limit: 10,
  ...
  const handleItemClick = (item) => {
    router.push(`/grupos/detalhes/${item.id_grupo}`);
  }
