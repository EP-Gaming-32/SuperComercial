...
export default function RegistrarOrdemCompraPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/pedidoFilial?id_filial=${filialSelecionada}&status=Pendente`)
      .then(r => r.json()).then(j => setPedidosFilial(j.data || []));
  }, [filialSelecionada]);
  ...
  const handleSelecionarPedido = pedido => {
    fetch(`http://localhost:5000/ordemCompra/itensPedidoFilial?id_pedido_filial=${pedido.id_pedido_filial}`)
      .then(r => r.json()).then(j => {
        const novos = j.data.map(item => ({
          id_produto: item.id_produto,
          nome_produto: item.nome_produto,
          quantidade: item.quantidade || 1,
          preco_unitario: 0,
        }));
        setProdutosOriginais(prev => [...prev, ...novos]);
      });
  };

  const handleSubmit = async ({ data_ordem, data_entrega_prevista, observacao, itens }) => {
    try {
      const payload = {
        data_ordem,
        ...
        itens // cada item já contém id_fornecedor, quantidade, preco_unitario
      };
